...
\begin{document}
	...
	\par 取两个滑块分别放在电子天平上，测得两滑块的质量分别为$m_1=230.21g$,$m_2=130.17g$,误差$\Delta m = 0.04g<0.1g$。打开气泵和数字毫秒计，等待一段时间后将滑块1放在导轨上并给予初速度，观察数字毫秒计上显示的滑块通过两次光电门的时间，若两次的时间差大于$0.1ms$,则保持旋钮$Q$不动，调节旋钮$P$，重复试验直到两者差值小于$0.1ms$为止。本次试验动态调平时时间分别为$104.61ms$,$104.64ms$。
	\begin{figure}[h]
		\centering
	...
	\end{figure}
	\subsection*{[数据处理]}
	\[\Delta s_1=0.05000m \quad \Delta s_2=0.05000m \quad \Delta s_1^{'}=0.05000m \]
	...
	\begin{table}[h]
		...
		\begin{tabular}{|l|llll|llll|}
			...
			& \multicolumn{1}{l|}{$\Delta t_1/ms$}     & \multicolumn{1}{l|}{$u/(m\cdot s^{-1})$} & \multicolumn{1}{l|}{$\Delta t_2$}     & $v$ & \multicolumn{1}{l|}{$\Delta t_1^{'}$}     & \multicolumn{1}{l|}{$u^{'}$} & \multicolumn{1}{l|}{$\Delta t_2^{'}$}     & $u^{'}$ \\ \hline
			1                    & \multicolumn{1}{l|}{110.53} & \multicolumn{1}{l|}{0.45236} & \multicolumn{1}{l|}{112.88} & 0.44294 & \multicolumn{1}{l|}{58.44} & \multicolumn{1}{l|}{0.85557} & \multicolumn{1}{l|}{118.57} & 0.42169 \\ \hline
			
			2                    & \multicolumn{1}{l|}{91.16} & \multicolumn{1}{l|}{0.54848} & \multicolumn{1}{l|}{93.53} & 0.53458 & \multicolumn{1}{l|}{63.71} & \multicolumn{1}{l|}{0.78480} & \multicolumn{1}{l|}{131.69} & 0.37967 \\ \hline
			
			3                    & \multicolumn{1}{l|}{82.38} & \multicolumn{1}{l|}{0.60694} & \multicolumn{1}{l|}{84.62} & 0.59087 & \multicolumn{1}{l|}{72.65} & \multicolumn{1}{l|}{0.68823} & \multicolumn{1}{l|}{146.52} & 0.34125 \\ \hline
		\end{tabular}
		\caption{时间记录及速度计算}
	\end{table}
	\subsubsection*{恢复系数e的计算}
	对于完全弹性碰撞，根据公式$e=\dfrac{v_2-v_1}{u_1-u_2}$易得
	\[e_1 = \frac{0.44294}{0.45236}=0.979181 \]
	\[e_2 = \frac{0.53458}{0.54848}=0.974660\]
	\[e_3 = \frac{0.59087}{0.60694}=0.973528\]
	\par 对于完全非弹性碰撞，由于两个滑块一起运动，故$e=0$
	\subsubsection*{动量和动能百分差}
	\par 对于完全弹性碰撞，根据公式(4)(5)计算可得
	\[E_{11}=0.02099\quad E_{12}=0.02551 \quad E_{13}=0.02664\]
	\[E_{21}=0.04137\quad E_{22}=0.05020 \quad E_{23}=0.05241\]
	\par 对于完全非弹性碰撞，根据公式(7)(8)计算可得
	\[E_{11}^{'}=0.01434 \quad E_{12}^{'}=0.03251 \quad E_{13}^{'}=0.00841\]
	\[E_{21}^{'}=0.51426 \quad E_{22}^{'}=0.53194 \quad E_{13}^{'}=0.50833\]
	\subsection*{[误差分析]}
	...
	\begin{figure}[h]
		\centering
		\includegraphics[width=1\linewidth]{实验数据}
